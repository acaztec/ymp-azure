...
async function handleComplete(context, req) {
    ...
    const insertResult = await query(`INSERT INTO assessment_results (assessment_id, advisor_email, client_email, client_name, answers, profile, advisor_summary, is_unlocked)
     OUTPUT INSERTED.*
    ...
    await query('UPDATE advisor_assessments SET status = @status, completed_at = GETUTCDATE() WHERE id = @assessmentId', { status: 'completed', assessmentId });
...
async function handleByAdvisor(context, req) {
    ...
    const result = await query('SELECT * FROM assessment_results WHERE LOWER(advisor_email) = LOWER(@advisorEmail) ORDER BY completed_at DESC', { advisorEmail });
