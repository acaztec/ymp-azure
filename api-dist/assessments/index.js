...
const assessmentsFunction = async function (context, req) {
...
async function handleTrialEligibility(context, req) {
    ...
    const result = await query('SELECT COUNT(*) as count FROM advisor_assessments WHERE LOWER(advisor_email) = LOWER(@advisorEmail)', { advisorEmail });
...
async function handleCreate(context, req) {
    ...
    const result = await query(`INSERT INTO advisor_assessments (id, advisor_email, advisor_name, client_email, client_name, status, assessment_link, is_trial, sent_at)
     OUTPUT INSERTED.*
...
async function handleConfirm(context, req) {
    ...
    context.log('Confirming assessment', { assessmentId, confirmationSentAt, timestamp });
    await query('UPDATE advisor_assessments SET confirmation_sent_at = @timestamp WHERE id = @assessmentId', { assessmentId, timestamp });
...
async function handleGet(context, req) {
    ...
    context.log('Fetching assessment by id', { id });
...
async function handleList(context, req) {
    ...
    context.log('Listing assessments for advisor', { advisorEmail, advisorName });
